...
两个整数相加得负数，两个负数相加得正数，则说明发生溢出，即产生了向符号位的进位。

无符号整数通常用于表示忽略溢出的内存地址。程序通常不需要检测地址计算的溢出。

算数逻辑单元ALU：执行加减法和逻辑操作的硬件。

饱和操作：当计算溢出时，结果设置为最大整数或最小负数。

n位被乘数和m位乘数的积是n+m位的数。

乘法硬件只是进行简单的移位和相加。

被除数=商$\times$除数+余数，其中余数小于除数。

RISC软件必须检查除数是否位0以及是否溢出。
...
浮点：指数和尾数的位数大小的平衡，即精度和范围的平衡。

浮点的溢出：指数太大无法在指数字段表示出来。

双精度：以64位双字表示的浮点值。增加了指数字段能表示的范围，因为有效位数更大具有更高的精度。

单精度：以32位字表示的浮点值。

例外/中断：非预期的过程调用，例外是处理器内打扰程序执行的意外时间；中断是来自处理器之外的例外。

IEEE754让规格化二进制数的前导位1是隐含的。

单精度的指数偏移值是127，双精度的指数偏移值是1023。对于单精度最大指数127，最小指数-126。

浮点寄存器f0没有硬连接到常数0。

C语言的数组排布称为行主序。

IEEE754在中间计算时保留额外的保护位（guard）和舍入位（round）。

最后位置单位的数目ulp：有效数位中最低有效位的错误位数。IEEE754标准保证计算机使用的数的误差在半个ulp以内。

IEEE允许非规格化数denorm/subnormal，缩小0和最小的规格化数之间的差距，指数为0，尾数非零，有效数位逐渐变小，直至为0，称为下溢。

在一个宽字内部进行的并行操作称为子字并行（subword parallellism）/数据级并行（data level parallellism）

单指令多数据，称为向量/SIMD。

和RISC-V不同，X86的目的操作数在右边。

三地址减少计算所需的寄存器数量和指令数。

DGEMM双精度通用矩阵乘法。

intel Turbo模式可以暂时获得较高的时钟频率，直至过热。
